...
class ui_compile
{
    ...
    public function parse_page($page = ''){
        ...
        foreach ($unique_js as $key => $c) {
            $js_content.=file_get_contents($c);
        }
        ...
        file_put_contents($this->tem_path.'cache/'.$page.'.css', $css_content);
        file_put_contents($this->tem_path.'cache/'.$page.'.js', $js_content);
	}
    ...
    public function parse_tag_static($type='css')
    {
        ...
        if($_M['form']['pageset'] || !$has){
            if(file_exists($res)){
                ...
                file_put_contents($new, $content.$add_content);
            }
        }
    }
    ...
    public function list_global_config($skin_name='')
    {
        ...
        foreach ($global as $v) {
            ...
            if($v['uip_key'] == 'bodybgimg' || $v['uip_key'] == 'met_font'){
                $val = $this->replace_m($v['uip_value']);
            }else{
    ...
    public function replace_attr($output)
    {
        global $_M;
        $that = $this;
        return preg_replace_callback("/(alt|value|title|placeholder|data-fv-message)=['\"]?([\S]+)?(<m[\s_a-zA-Z=\d>-]+<\/m>)['\"]?/isu", function($match) use ($that){
