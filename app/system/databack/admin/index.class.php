...
class index extends admin {
	...
	public function doimport(){
		...
		if(strstr($admin_op['admin_op'],'metinfo') === false){
			echo "<script type='text/javascript'> alert('{$_M[word][jsx38]}');location.href='recovery.php?anyid={$anyid}&lang={$lang}&cs=2'; </script>";
			die();
		}
		$fileid = $fileid ? $fileid : 1;
		$filename = $_M[form][pre].$fileid.'.sql';
		$filepath = PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$filename;
	...
	public function dosql_execute(){
		...
		if(file_exists($filepath)){
			$sql = file_get_contents($filepath);
			if(substr($sql,28,5)!=$_M[config][metcms_v] && substr($sql,28,6)!=$_M[config][metcms_v])turnover($rurls,$_M[word][dataerr1]);
	...
	public  function dogetfileattr(){
		...
		foreach($infos as $key=>$val){
			$fp = @fopen(PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$val['filename'],"rb");
			$str = @fgets($fp);
			@fclose($fp);
		...
		foreach($infos as $key=>$val){
			...
			$info_num=1;
			while(file_exists(PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$key.$info_num.'.sql')){
				$info_num++;
			}
   ...
   function docache_write($file, $string, $type = 'array'){
		...
		file_put_contents(PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$file, $string);
	}
   ...
   function dotableprearray($tablepre){
	...
	function dosql_dumptable($table, $startfrom = 0, $currsize = 0){
  	...
  	function docache_delete($file){
		global $_M;
		return @unlink(PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$file);
	}
	...
	/*上传文件备份*/
	 function dopackupload(){
		...
		if(!file_exists(PATH_WEB.$_M['config']['met_adminfile'].'/databack/upload'))@mkdir (PATH_WEB.$_M['config']['met_adminfile'].'/databack/upload', 0777);
		$sqlzip=PATH_WEB.$_M['config']['met_adminfile'].'/databack/upload/'.$_M[config][met_agents_backup].'_upload_'.date('YmdHis',time()).'.zip';
		$zipfile=PATH_WEB."upload";
		$archive = new PclZip($sqlzip);
		$zip_list = $archive->create($zipfile,PCLZIP_OPT_REMOVE_PATH,PATH_WEB);
		if ($zip_list == 0) {
			//die("Error : ".$archive->errorInfo(true));
			turnover("{$_M[url][own_form]}a=doindex",$_M[word][setdbArchiveNo]);
		}
	...
	public function dogetsql($table='') {
		...
		$startfrom = isset($_M['form']['startfrom']) ? intval($startfrom) : 0;
		...
		if(trim($sqldump)){
			...
			$fileid++;
			$bakfile = PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$filename;
			if(!is_writable(PATH_WEB.$_M['config']['met_adminfile'].'/databack/'))turnover("{$_M[url][own_form]}a=doindex",$_M[word][setdbTip2].'databack/'.$_M[word][setdbTip3]);
			file_put_contents($bakfile, $sqldump);
			if(!file_exists(PATH_WEB.$_M['config']['met_adminfile'].'databack/sql'))@mkdir (PATH_WEB.$_M['config']['met_adminfile'].'/databack/sql', 0777);
			$sqlzip=PATH_WEB.$_M['config']['met_adminfile'].'/databack/sql/'.$_M[config][met_agents_backup].'_'.$zipname.'.zip';
			$archive = new PclZip($sqlzip);
			$zip_list = $archive->create(PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$filename,PCLZIP_OPT_REMOVE_PATH,PATH_WEB.$_M['config']['met_adminfile'].'/databack/');
		}
		if(trim($sqldump)){
  	...
  	function doallfile(){
	    global $_M;
	    $this->dogetsql();
		if(!file_exists(PATH_WEB.$_M['config']['met_adminfile'].'/databack/web'))@mkdir (PATH_WEB.$_M['config']['met_adminfile'].'/databack/web', 0777);

		$sqlzip=PATH_WEB.$_M['config']['met_adminfile'].'/databack/web/'.$_M[config][met_agents_backup].'_web_'.$con_db_name.'_'.date('YmdHis',time()).'_'.met_rand(6).'.zip';

		//$zipfile='../';
		$archive = new PclZip($sqlzip);
		$zip_list = $archive->create(PATH_WEB,PCLZIP_OPT_REMOVE_PATH,PATH_WEB,PCLZIP_OPT_ADD_PATH,'web');
		if($zip_list==0){
			die("Error : ".$archive->errorInfo(true));
			turnover("{$_M[url][own_form]}a=doindex",$_M[word][setdbArchiveNo]);
		}
	...
	public function dodelete(){
		...
		if(trim(substr(strrchr($_M[form][filenames], '.'), 1))=='zip'){
			@unlink(PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$_M[form][fileon].'/'.$_M[form][filenames]);
			turnover("{$_M[url][own_form]}a=dodownload",$_M[word][physicaldelok]);
		}else{
			$prefix=$_M[form][filenames];
			$sqlfiles = glob(PATH_WEB.$_M['config']['met_adminfile'].'/databack/*.sql');
			foreach($sqlfiles as $id=>$sqlfile){
				$sqlfile=str_ireplace(PATH_WEB.$_M['config']['met_adminfile'].'/databack/','',$sqlfile);
				if(stripos($sqlfile,$prefix)!==false){
					$filetype=trim(substr(strrchr($sqlfile, '.'), 1));
					if($filetype=='sql'){
						$filenamearray=explode(".sql",$sqlfile);
						@unlink(PATH_WEB.$_M['config']['met_adminfile'].'/databack/'.$sqlfile);
						@unlink(PATH_WEB.$_M['config']['met_adminfile'].'/databack/sql/'.$_M[config][met_agents_backup].'_'.$filenamearray[0].".zip");
					}
				}
			}
		}
	...
	public function dogetfilefix(){
		 global $_M;
		 $sqlfiles = glob(PATH_WEB.$_M['config']['met_adminfile'].'/databack/sql/*.zip');
		 if(is_array($sqlfiles)){
			 $prepre = '';
			 $info = $infos = array();
			 foreach($sqlfiles as $id=>$sqlfile){
		 ...
		 $sqlfiles = glob(PATH_WEB.$_M['config']['met_adminfile'].'/databack/config/*.zip');
		 if(is_array($sqlfiles))
		 {
			 $prepre = '';
			 $info = $infos = array();
...
/*upload*/
		 $sqlfiles = glob(PATH_WEB.$_M['config']['met_adminfile'].'/databack/upload/*.zip');
		 if(is_array($sqlfiles))
		 {
			 $prepre = '';
			 $info = $infos = array();
...
/*all files*/
		 $sqlfiles = glob(PATH_WEB.$_M['config']['met_adminfile'].'/databack/web/*.zip');
		 if(is_array($sqlfiles))
		 {
			 $prepre = '';
			 $info = $infos = array();
